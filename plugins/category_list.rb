  ...
  class CategoryCloud < Liquid::Tag
    ...
    def render(context)
      ...
      lists.each do | category, counter |
        ...
        text_class = text_class(counter)
        style = "font-size: #{font_size}%;"
        html << "<a class='#{text_class}' href='#{url}' style='#{style}'>#{category}"
        if @opts['counter']
          html << "<span class='category-counter badge'>#{categories[category].count}<span>"
        end
        html << "</a> "
      end
      html
